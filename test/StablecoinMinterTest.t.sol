...
contract StablecoinMinterTest is Test {
    ...
    function testWithdrawCollateralRevertsIfLockNotExpired() external {
        ...
        vm.startPrank(user);
        vm.expectRevert(StablecoinMinter.StablecoinMinter__LockStillAlive.selector);
        stablecoinMinter.withdrawCollateral(mockCollateral1);
        vm.stopPrank();
    }

    function testWithdrawCollateralRevertsIfNoCollateralAfterExpiration() external {
        // 1. Lock collateral for 30 days
        uint256 lockAmount = 1 ether;
        vm.startPrank(user);
        stablecoinMinter.lockCollateral(mockCollateral1, lockAmount, 30 days);
        ...
        bytes32 userFinalCollateralSlot = keccak256(abi.encode(mockCollateral1, userCollateralSlot));
        vm.store(address(stablecoinMinter), userFinalCollateralSlot, bytes32(uint256(0)));

        // 4. Attemp to withdraw
        vm.startPrank(user);
        vm.expectRevert(StablecoinMinter.StablecoinMinter__NoCollateralToWithdraw.selector);
        stablecoinMinter.withdrawCollateral(mockCollateral1);
        vm.stopPrank();
    }
}
